    ...
    <div className="panel flex flex-col h-full min-h-0 overflow-hidden">
      ...
      <div className="overflow-y-auto min-h-0 flex-1">
        {transactions.length === 0 ? (
          <div className="px-4 py-8 text-center text-[12px] text-[var(--muted)] tracking-widest">
          ...
          transactions.map((tx) => (
            ...
            >
              {/* TX icon */}
              <div className="w-8 h-8 rounded-sm bg-[#1a1800] flex items-center justify-center shrink-0">
              ...
              <div className="flex flex-col min-w-0 flex-1">
                <a
                  ...
                  className="text-[12px] font-mono text-[var(--cyan)] hover:underline truncate"
                >
                  {truncate(tx.hash, 8)}
                </a>
                <div className="flex items-center gap-1 text-[11px] text-[var(--muted)] tracking-wider">
                  <a
                    href={`https://explore.tempo.xyz/address/${tx.from}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="font-mono hover:text-[var(--cyan)] transition-colors"
                  >
                    {truncate(tx.from, 4)}
                  </a>
              ...
              <div className="flex flex-col items-end shrink-0">
                <span className="text-[12px] font-mono text-[var(--yellow)] font-bold">
